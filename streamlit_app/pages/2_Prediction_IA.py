...
# ========= CONFIG & STYLES =========
st.set_page_config(
    page_title="Prédiction DPE & Interprétation IA",
    page_icon=logo_path,
    layout="wide"
)
...
# === CHOIX DU MODE ===
st.markdown("##  Choisissez le type de prédiction")
...
if not pred or pred.get("mode") != "complet":
    st.info(" Lancez d’abord une prédiction complète avant de demander une interprétation.")
    st.stop()
...
if st.button(" Générer l’interprétation détaillée", use_container_width=True, type="primary"):
    ...
    with st.spinner("Mistral analyse votre logement..."):
        try:
            ...
            if "429" in str(e):
                st.error("Trop de requêtes envoyées à Mistral AI. Patientez quelques secondes avant de réessayer.")
            else:
